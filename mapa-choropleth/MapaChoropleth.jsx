...
export default function MapaChoropleth({
  ...
  const totalesPorMun = useMemo(() => {
    ...
    if (delito) fil = fil.filter((r) =>
      r["Tipo de delito"]?.toLowerCase().includes(delito.toLowerCase())
    );
    const mapa = {};
    fil.forEach((r) => {
  ...
  }, [geoJson, totalesPorMun, escalaColor, selected]);
    ...
    return (
      ...
      >
        ...
        <p className="text-blue-300">
          ...
          <span className="text-gray-400">carpetas</span>
        </p>
        {selected === municipio && (
      ...
      {selected && !loading && !error && (
        ...
        >
          ...
          <p className="text-blue-300 text-xs">
            {(totalesPorMun[selected] || 0).toLocaleString("es-MX")} carpetas
          </p>
          <button
            onClick={() => { setSelected(null); onMunSelect?.(null); }}
